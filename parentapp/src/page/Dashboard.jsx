...
import { 
  ...
  where, getDocs, limit, updateDoc, arrayUnion 
} from "firebase/firestore";
// --- Messaging Import: Iska path sahi rakhiyega ---
...
export default function Dashboard() {
  ...
  // --- NEW: FCM Token Setup (Permission & Database Sync) ---
  useEffect(() => {
    const setupFCM = async () => {
      // 1. Pehle check karein ki kya login hai
      ...
      try {
        ...
        // 2. requestForToken call hote hi browser pop-up dikhayega
        const token = await requestForToken(); 
        
        if (token) {
          const parentRef = doc(db, "parents", pid);
          // 3. Token ko Firestore mein save karein
          await updateDoc(parentRef, {
            fcmTokens: arrayUnion(token)
          });
          console.log("FCM Token synced successfully!");
        } else {
          console.log("Token generation failed or permission denied.");
        }
      } catch (err) {
        console.error("FCM Error in Dashboard:", err);
      }
    };

    // Chhoti si delay ke saath call karein taaki browser ready ho jaye
    ...
    <div className="min-h-screen bg-slate-50 p-4 pb-24 font-sans max-w-md mx-auto relative">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <MenuCard
          ...
          onClick={() => navigate("/profile", { state: { student: liveStudent } })}
        />
